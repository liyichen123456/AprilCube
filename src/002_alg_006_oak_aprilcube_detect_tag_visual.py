...
WINDOW_PREFIX = "OAK AprilCube alg006"
ENABLE_VISER = True
...
ALG006_SUPPORT_ROTATION_THRESH_DEG = 35.0
ALG006_SUPPORT_TRANSLATION_THRESH_MM = 25.0
...
class AprilCubeAlg006PoseRuntime(AprilCubeTemporalPoseRuntime):
    """Runtime that applies benchmark alg_06 to live OAK frames, without temporal filtering."""

    def detect_native_apriltags_corners_only(self, image: np.ndarray) -> list[Any]:
        gray = self.prepare_native_detection_gray(image)
        tags = self.native_detector.detect(gray, estimate_tag_pose=False)
        return self._filter_valid_tags(list(tags))
    ...
    def _empty_result() -> dict[str, Any]:
        return {
            ...
            "tag_pose_by_id": {},
            "cube_candidate_debug": {"mode": "alg_006_cube_consistency_ippe"},
        }
    ...
    def process_frame(
        ...
        if native_tags is None:
            native_tags = self.detect_native_apriltags_corners_only(image)
        else:
        ...
        for tag in native_tags:
            tag_id = int(tag.tag_id)
            tag_candidates = estimate_tag_pose_candidates_solvepnp_ippe(self.detector, tag)
            for tag_candidate in tag_candidates:
                tag_rot_mat = np.asarray(tag_candidate["pose_R"], dtype=np.float64).reshape(3, 3)
                tag_tvec_mm = np.asarray(tag_candidate["pose_t"], dtype=np.float64).reshape(3, 1) * 1000.0
                cube_pose = cube_pose_from_tag_pose(self.detector, tag_id, tag_rot_mat, tag_tvec_mm)
                ...
                cube_candidates.append(
                    {
                        "tag_id": tag_id,
                        "candidate_index": int(tag_candidate.get("candidate_index", -1)),
                        "rot_mat": cube_rot_mat,
                        "tvec": cube_tvec,
                        "reproj_error": tag_candidate.get("reproj_error_px", None),
                        "tag_rot_mat": tag_rot_mat,
                        "tag_tvec": tag_tvec_mm,
                    }
                )

        result["cube_candidate_debug"] = {
            "mode": "alg_006_cube_consistency_ippe",
            "raw_candidate_count": int(len(cube_candidates)),
            "selected_candidate_count": 0,
            "support_rotation_thresh_deg": float(ALG006_SUPPORT_ROTATION_THRESH_DEG),
            "support_translation_thresh_mm": float(ALG006_SUPPORT_TRANSLATION_THRESH_MM),
        }
        if not cube_candidates:
            return self.detector._store_latest(result, image)
        ...
        for anchor in cube_candidates:
            support: list[dict[str, Any]] = []
            score = 0.0
            anchor_rot = np.asarray(anchor["rot_mat"], dtype=np.float64).reshape(3, 3)
            anchor_tvec = np.asarray(anchor["tvec"], dtype=np.float64).reshape(3, 1)
            for cand in cube_candidates:
                rot_err = rotation_angle_deg_between(anchor_rot, np.asarray(cand["rot_mat"], dtype=np.float64))
                trans_err = float(
                    np.linalg.norm(
                ...
                if (
                    rot_err <= float(ALG006_SUPPORT_ROTATION_THRESH_DEG)
                    and trans_err <= float(ALG006_SUPPORT_TRANSLATION_THRESH_MM)
                ):
                    support.append(cand)
        ...
        result["cube_candidate_debug"].update(
            {
                "selected_candidate_count": int(len(best_support)),
                "selected_tag_count": int(len({int(c["tag_id"]) for c in best_support})),
                "best_score": float(best_score),
            }
        )
        fused = fuse_cube_pose_candidates(best_support)
        if fused is None:
            return self.detector._store_latest(result, image)

        cube_rot_mat, cube_tvec = fused
        cube_rvec, _ = cv2.Rodrigues(cube_rot_mat)
        cube_center = np.asarray(cube_tvec, dtype=np.float64).reshape(3)

        inward_count = 0
        invalid_count = 0
        selected_by_tag: dict[int, dict[str, Any]] = {}
        for cand in best_support:
            tag_id = int(cand["tag_id"])
            prev = selected_by_tag.get(tag_id)
            cand_reproj = cand.get("reproj_error", None)
            prev_reproj = prev.get("reproj_error", None) if prev is not None else None
            cand_score = float(cand_reproj) if cand_reproj is not None else float("inf")
            prev_score = float(prev_reproj) if prev_reproj is not None else float("inf")
            if prev is None or cand_score < prev_score:
                selected_by_tag[tag_id] = cand

        for tag_id, cand in selected_by_tag.items():
            tag_rot_mat = np.asarray(cand["tag_rot_mat"], dtype=np.float64).reshape(3, 3)
            tag_tvec = np.asarray(cand["tag_tvec"], dtype=np.float64).reshape(3, 1)
            inward_ok = tag_z_points_to_cube_interior(tag_rot_mat, tag_tvec, cube_center)
            if inward_ok:
                inward_count += 1
            else:
                invalid_count += 1
        ...
        result["n_inliers"] = int(len(best_support) * 4)
...
def main() -> None:
    ...
    print("[INFO] Pose algorithm: alg_006 IPPE tag candidates -> cube consistency support selection")
    ...
    cube_runtimes = {
        camera_name: AprilCubeAlg006PoseRuntime(
            detector=detectors[camera_name],
