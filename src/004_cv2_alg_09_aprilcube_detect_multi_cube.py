...
from aprilcube_pose_benchmark.alg_09_cube_candidate_cluster_ransac_temporal_sanity import (  # noqa: E402
    algorithm_fn as alg09_algorithm_fn,
)
...
# Native pupil_apriltags detection settings. alg_09 only needs native-detected
# tag corners; pose candidates are generated by the alg_09 selection logic.
...
class AprilCubeAlg09PoseRuntime(AprilCubeTemporalPoseRuntime):
    """Use pupil_apriltags for corners, then benchmark alg_09 pose selection."""
    ...
    def process_frame(
        ...
        debug = result.setdefault("algorithm_debug", {})
        debug["algorithm_name"] = ALGORITHM_NAME
        debug["native_raw_tag_count"] = int(raw_count)
        debug["native_valid_tag_count"] = int(len(native_tags))

        if result.get("success", False) and not result.get("predicted", False):
            self.detector.prev_rvec = np.asarray(result["rvec"], dtype=np.float64).reshape(3, 1).copy()
            self.detector.prev_tvec = np.asarray(result["tvec"], dtype=np.float64).reshape(3, 1).copy()

        return self.detector._store_latest(result, image)

    def __init__(self, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self._alg09_context: dict[str, Any] = {}
...
def main() -> None:
    ...
    for cube_path in cube_paths:
        ...
        for camera_name in active_camera_names:
            ...
            runtime = AprilCubeAlg09PoseRuntime(
                detector=detector,
