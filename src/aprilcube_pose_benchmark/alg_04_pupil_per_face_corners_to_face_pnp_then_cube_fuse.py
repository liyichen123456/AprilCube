...
from aprilcube_pose_benchmark.common_pose import detections_from_tags, per_face_pnp_fusion
...
ALGORITHM_NAME = "alg_04_pupil_per_face_corners_to_face_pnp_then_cube_fuse"


def algorithm_fn(detector: Any, native_detector: Any, tags: list[Any], gray: np.ndarray, context: dict[str, Any]) -> dict[str, Any]:
    """按 face 分组，每个 face 单独做 cube PnP，再融合多个 face 的 cube pose。"""
    detections = detections_from_tags(tags)
    return per_face_pnp_fusion(detector, detections)
